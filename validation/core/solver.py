...
VPY samplers for modes 1/2/3, trap_quality.

solver.py — CP-SAT движок решаемости (требование R2): однозначность разбивки,
VPY-сэмплеры для mode-1/2/3, trap_quality.
"""
...
def count_partitions(words, members, cap=2) -> int:
    """Count partitions of W into monochromatic quads (CP-SAT exact cover). cap is an
    enumeration cutoff: returns min(true count, cap). Default cap=2: the verdict only
    needs three outcomes — 0 (unsolvable), 1 (unambiguous, certified), >=2 (ambiguous,
    invalid — the exact count no longer matters).
    ...
    невалиден — точное число уже не влияет).
    In: words (|W| кратно 4), members, cap. Out: int 0..cap."""
    ...
    class _C(cp_model.CpSolverSolutionCallback):
        ...
        def on_solution_callback(self):
            """Count solutions and stop at the cap. / Считает решения; стоп на лимите."""
            self.count += 1
            if self.count >= cap:
...
def is_unique_puzzle(words, members) -> bool:
    """Return True if the puzzle is unique: exactly one partition exists.

    In: words, members. Out: bool.

    Однозначен ли пазл: ровно одна разбивка.
    In: words, members. Out: bool."""
    return count_partitions(words, members, cap=2) == 1


def _ceiling_mode1(members) -> int:
    """Combinatorial ceiling of mode-1 boards: for every quadruple of categories,
    multiply the number of ways to pick 4 terms from each (C(size, 4)), then sum
    over all quadruples.
...
def sample_valid_puzzles(members, term_tags, n_samples=400, seed=42) -> dict:
    """mode-1: sample 4-category × 4-term boards and certify uniqueness.

    In: members, term_tags, n_samples, seed.
    Out: dict {vpy, exists, ceiling, effective, …}.
...
def _draw_partition(tags, members, rng):
    """Draw 16 distinct words from 4 categories (4 per category, no reuse).
    ...
    In: tags (list), members, rng. Out: list[str] | None (если не извлекается)."""
    used: set = set()
    words = []
    for a in tags:
...
def sample_valid_puzzles_mode2(members, term_tags, n_samples=400, seed=42) -> dict:
    """mode-2: a candidate W must contain a term holding ≥ 2 tags within the chosen
    four categories (overlap trap); valid = is_unique.

    In: members, term_tags, n_samples, seed.
    Out: dict {vpy, exists, samples_drawn, …}.

    mode-2: кандидат W обязан содержать термин с ≥ 2 тегами из выбранной четвёрки
    (ловушка-пересечение); valid = is_unique.
    In: members, term_tags, n_samples, seed. Out: dict {vpy, exists, samples_drawn, …}."""
...
def trap_quality(words, members, members_r3) -> int:
    """Count quads monochromatic under R₃ = member_of ∪ decoy_for but NOT under
    member_of: each such quad exists only because of decoy edges.

    In: words, members, members_r3. Out: int (count of such quads).

    Квадры, монохромные под R₃ = member_of ∪ decoy_for, но НЕ под member_of:
    неверная четвёрка существует только из-за decoy-рёбер.
    In: words, members, members_r3. Out: int (число таких квадр)."""
...
def sample_valid_puzzles_mode3(members, term_tags, decoy_map,
    ...
    valid = is_unique(member_of) ∧ trap_quality ≥ 1.

    In: members, term_tags, decoy_map (term→set(tags)), n_samples, seed.
    Out: dict {vpy, exists, decoy_pool, …}.

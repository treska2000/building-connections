...
def _slug(text: str) -> str:
    """Kebab-case slug from a string.

    In: text. Out: str (never empty).

    Kebab-case slug из строки.
    In: text. Out: str (не пустой)."""
    s = re.sub(r"[^a-z0-9]+", "-", (text or "").strip().lower()).strip("-")
    return s or "config"


def _src_url(s: str) -> str:
    """arXiv id → URL; ready URLs pass through unchanged.

    In: s. Out: str.

    arXiv-id → URL; готовые URL — без изменений.
    In: s. Out: str."""
...
def pool_v2_to_config(pool: dict) -> dict:
    """Normalize the native v2 pool into a v1-like config, mirroring
    connections_v2.export_input: term tags = home_category first +
    candidate_categories, labels, dedup.

    In: pool (v2 pool dict). Out: config dict (+ _pool_validation as reference).
    ...
    In: pool (dict v2-пула). Out: dict конфига (+ _pool_validation справочно)."""
...
def load_config(path: str) -> dict:
    """Read a JSON config; a v2 pool is auto-detected by schema_version and normalized.

    In: path. Out: config dict.

    Читает JSON-конфиг; v2-пул авто-детектится по schema_version и нормализуется.
    In: path. Out: dict конфига."""
...
def _norm(s: str) -> str:
    """Normalize a name: lowercase + whitespace collapsing.

    In: s. Out: str.

    Нормализация имени: lower + схлопывание пробелов.
    In: s. Out: str."""
    return " ".join((s or "").lower().strip().split())


def get_term_tags(term: dict) -> list[str]:
    """Return a term's categories (tags, falling back to axes).

    In: term. Out: list[str].

    Категории термина (tags, fallback axes).
    In: term. Out: list[str]."""
    return list(term.get("tags") or term.get("axes") or [])


def get_decoys(term: dict) -> list[str]:
    """Return a term's decoy targets (decoy_for_tags, falling back to decoy_for_axes).

    In: term. Out: list[str].

    Decoy-цели термина (decoy_for_tags, fallback decoy_for_axes).
    In: term. Out: list[str]."""
    return list(term.get("decoy_for_tags") or term.get("decoy_for_axes") or [])


def get_category_names(cfg: dict) -> list[str]:
    """Return the config's category names (objects or bare strings).

    In: cfg. Out: list[str].

    Имена категорий конфига (объекты или строки).
    In: cfg. Out: list[str]."""
...
def get_area(cfg: dict) -> str | None:
    """Return the config's area: specialty.area (object form) or the specialty string itself.
...
def get_sources(term: dict) -> list[dict]:
    """Return a term's sources (sources / evidence / top_sources), url-bearing entries only.
...
def dedup_report(cfg: dict) -> dict:
    """Count duplicates: of terms (by normalized name) and of (term, category) edges.
...
def build_membership(cfg: dict):
    """Build the membership graph.

    In: cfg. Out: (members: tag→set(terms), term_tags: term→set(tags)) —
    set semantics deduplicate edges.

    Строит граф принадлежности.
    In: cfg. Out: (members: tag→set(термины), term_tags: термин→set(теги)) — дедуп по множествам."""
...
def tag_sizes(members: dict[str, set[str]]) -> Counter:
    ...
    In: members. Out: Counter tag→size.

    Размеры категорий.
    In: members. Out: Counter tag→размер."""
    return Counter({a: len(v) for a, v in members.items()})


def list_to_config(raw, name: str, field: str | None = None) -> dict:
    """Normalize a bare term-list pool ([{name, tags, description}]) into a config
    dict that build_membership accepts; pass dicts through unchanged.

    In: raw (list or dict), name (str), field (str | None). Out: dict.

    Нормализует «голый список» термов в dict-конфиг; dict возвращает как есть.
    In: raw (list или dict), name (str), field (str | None). Out: dict."""
    if isinstance(raw, dict):
        return raw
    tags = sorted({t for term in raw for t in (term.get("tags") or [])})
    specialty = {"area": name.replace("_", " ").replace("-", " ")}
    if field:
        specialty["field"] = field
    return {"config_id": f"{name}-v2", "config_version": "2.0.0",
            "specialty": specialty, "tags": tags, "terms": raw}
